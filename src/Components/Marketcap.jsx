...
export default function Marketcap() {
  ...
  return (
    <div className="w-full h-screen lg:h-[778px] text-white shadow-sm transition ease-in-out hover:ring-4 rounded-md ring-white/60 ring-offset-slate-50 overflow-hidden bg-blue-500 bg-opacity-50 backdrop-blur-md">
      <div className="sticky top-0 z-10 bg-cyan-600 bg-opacity-10 backdrop-blur-md">
        ...
        {cryptocurrencies.map((currency, i) => (
          <div key={i} className="border-b px-6 py-4 max-sm:px-3 max-sm:py-2 flex justify-between items-center">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-black">Mkt.Cap ₹{currency.market_cap}</p>
            </div>
            <div className="flex justify-center items-center">
              {currency.price_change_percentage_24h > 0 && (
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-green-500 font-semibold pl-2">{currency.price_change_percentage_24h.toFixed(2)}%</p>
                </div>
              )}
              {currency.price_change_percentage_24h < 0 && (
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-orange-500 font-semibold pl-1">{currency.price_change_percentage_24h.toFixed(2)}%</p>
                </div>
              )}
            </div>
          </div>
